...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/03/14 11:19:57 by madufour          #+#    #+#             */
/*   Updated: 2019/03/18 12:28:05 by madufour         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fractol.h"

static int		fl_mandel(t_mlx *mlx, int x, int y, t_conf c)
{
	t_compl	t;
	int		i;
	double	tmp;
...
static void		*fl_mandel_thread(void *param)
{
	t_data	*data;
	int		x;
	int		i;
	...
	{
		i = fl_mandel(data->mlx, x, data->y, data->c);
...
static void		fl_fill_img(t_mlx *mlx)
{
	...
	while (y < mlx->size_y)
	{
		...
		pthread_create(&(th.thread[y]), NULL, fl_mandel_thread, &(th.data[y]));
		y++;
	}
	y = 0;
	while (y < mlx->size_y)
