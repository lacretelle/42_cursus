...
exports.getNotifs = async function(data) {
	let q = 'SELECT notif.type, user.username, image.path AS img, notif.date FROM `notif` INNER JOIN image ON notif.id_author = image.id_user AND image.profil = 1 AND notif.id_receiver = ? LEFT JOIN user ON user.id_user = notif.id_author ORDER BY notif.date DESC';
	try {
		let res = await pool.query(q, data);
		return (res);
	} catch (err) {
...
exports.getOthersChat = async function (data) {
	...
	let q = 'SELECT user.id_user, user.username, image.path AS img FROM user INNER JOIN image ON USER.id_user = image.id_user AND USER.id_user IN('+ids+') AND image.profil = 1 AND USER.id_user IN('+ids+')';
	try {
...
exports.find_withid = async function (data) {
	var q = 'SELECT mail, lastname, firstname, gender, sexpref, bio, birthdate, city, latitude, longitude, status FROM `user` WHERE id_user = ?';
	try {
		let res = await pool.query(q, data);
		return(res[0]);
	} catch (err) {
