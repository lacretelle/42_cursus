...
export default function Notif() {
	...
	useEffect(() => {
		socket.on('notif', function(data) {
			if (data.type === 'visit')
				dispatchSearch({type: 'NEW_VISIT', payload: [...search.visits, {id_visitor: data.id_visitor, id_visited: state.id_session}]
				});
			...
			else if (data.type === 'unlike')
			{
				...
				if (data.delete)
					tmpMatchs = tmpMatchs.filter(e =>
						(e.id_user1 !== data.id_author
							&& e.id_user2 !== data.id_author));
	...
	async function watchNotif() {
		...
		else
		{
			...
			let oldNotifs = await fetchOldNotif(state.id_session);
			setNotif(oldNotifs);
		}
		if (countNotif > 0)
			setCountNotif(0);
			...
			<FontAwesomeIcon icon={faBell} className='header-icon' />
				...
				<ul id='notif-wrapper'>
						{notif ? notif.map((e, index) => (<li key={index} className='notif'>
							<img className='notifUserPic' src={e.img} alt={`${e.id_author}-mainPic`} />
...
async function fetchOldNotif(id)
{
	try {
		let res = await axios.get(`/user/notif/${id}`);
		if (res.data)
			return (res.data);
		else
			return (null);
