...
import { faEye, faCircle,
	faHeart, faExclamationTriangle} from '@fortawesome/free-solid-svg-icons';
...
export default function Gallery(props) {
	...
	//DISPLAY 10 MORE PROFIL ON SCROLL
	useEffect(() => {
		function toggleScroll() {
			...
			let scrollPercent = (scrollTop / (scrollHeight - windowHeight)) * 100;
			if (scrollPercent >= 100)
				setI(i + 1);
		}
		if ((i + 1) * 10 < targother.length)
	...
	}, [props.others, search, visited, liked, state, sortSearch, ascSort])

	//MAKE A LIMITED COPY BASED ON I OF TARGOTHER TO DISPLAY
	useEffect(() => {
		if (targother.length > 0)
											...
											<div className="lineToggle">
												...
												<ul className="listOthers">
														{tmpTargother && tmpTargother.map((e, index) => (<li className='others' key={index} value={e.id_user} onClick={() => watchProfil(e)}>
															...
															<div className='othersInfos'> <FontAwesomeIcon icon={faCircle} style={{color: e.online === true ? 'green' : 'grey'}} /> {e.username}<br/>{e.age} ans, {e.city}</div></li>))}
														</ul>
															...
															<div className='modal-content'>
																...
																<div className='profilOther'>
																	<div className='profil-img'>
																		<div className='imgP'>
																			<h3>{other.username} {other.online === true ? 'En ligne' : `Dernière connexion le : ${troncDate(other.lastco)}`} </h3>
																				{other.imgs && other.imgs.map((e, index) => (<img src={e.path} alt={'profilpic' + index} key={index}/>))}
																			</div>
																			<div>
																					{ search.visits && search.visits.find(e => (e.id_visitor === other.id_user)) && <p><FontAwesomeIcon icon={faEye} className="interactionIcon" /></p>}
																						{search.likes.find(e => (e.id_author === other.id_user)) && <p><FontAwesomeIcon icon={faHeart} className="interactionIcon"/></p>}
																						<p>Score: {other.popular}</p>
																						<FontAwesomeIcon icon={faExclamationTriangle} onClick={report} className='reportIcon' />
																						<div id="report">
																							<ul>
																							...
																							{(other.imgs && other.imgs.length > 0) && <FontAwesomeIcon icon={faHeart} onClick={toggleLike} className='likeButton' style={{color: search.likes.find(e => (e.id_liked === other.id_user)) ? 'red' : 'green'}}/>}
																						</div>

																						<div className='profil-info'>
																							<div>
																								<h3>Je suis {other.firstname} {other.lastname}, {other.age} ans</h3>
																							...
																							<div>
																								<h3>Quelques informations personnelles: </h3>
																									...
																									<div className='map'>
																										<Map others={targother} coord={[search.lat, search.lon]} />
																									</div>
																							</>
	)
}
...
function troncDate(date) {
	if (date)
		date = `${date.slice(0, 10)} à ${date.slice(11, 19)}`;
	return (date);
}
